...
;; 
used_memory_human)
        result=`$REDIS_INFO|/bin/grep used_memory_human|awk -F":" '{print $NF}'|awk -F'K' '{print $1}' |awk -F'K|M|G' '{print $1}'`
        echo $result 
;; 
used_memory_rss)
        result=`$REDIS_INFO|/bin/grep used_memory_rss|awk -F":" '{print $NF}' |awk -F":" '{print $NF}'|awk 'NR==1'`
        echo $result 
;; 
used_memory_peak)
...
used_memory_peak_human)
        result=`$REDIS_INFO|/bin/grep used_memory_peak_human|awk -F":" '{print $NF}'|awk -F'K' '{print $1}'|awk -F'K|M|G' '{print $1}'`
        echo $result 
;; 
used_memory_lua)
...
;;
keyspace_hits_rate)
        hits=`$REDIS_INFO|/bin/grep -w "keyspace_hits" | awk -F':' '{print $2}' | sed -n 's/\\r//p'` 
        miss=`$REDIS_INFO|/bin/grep -w "keyspace_misses" | awk -F':' '{print $2}' | sed -n 's/\\r//p'`
	total=$((${hits}+${miss}))
	hits_rate=`echo "scale=2;(${hits}/$total)*100" | bc`
	echo $hits_rate
;;
pubsub_channels)
